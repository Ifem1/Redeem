...
class Redeem(gl.Contract):
 ...
 def _validate(self,sources:str,outcomes:str):
  ...
  for s in ss:
   assert set(s.keys())=={"label","url","authority","required"} and isinstance(s["label"],str) and 0<len(s["label"])<=100 and isinstance(s["required"],bool) and s["authority"] in ("PRIMARY","CORROBORATING") and isinstance(s["url"],str) and s["url"].startswith("https://") and len(s["url"])<=500 and "@" not in s["url"] and s["url"] not in urls;host=s["url"][8:].split("/")[0].lower();assert host not in ("localhost","127.0.0.1","::1") and not host.startswith("127.") and not host.startswith("10.") and not host.startswith("192.168.");urls.append(s["url"]);primary=primary or s["authority"]=="PRIMARY"
  assert primary
  for o in oo:
 ...
 def _apply(self,i:u256,challenge:bool):
  ...
  g.review_attempts=round;g.last_review_status=r["status"]
 ...
 def evaluate_redemption(self,i:u256):
  g=self._get(i);assert g.status in (u8(OPEN),u8(RETRYABLE)) and g.review_attempts<u8(MAX_RETRIES);self._apply(i,False)
 @gl.public.write.payable
 ...
 @gl.public.write
 def resolve_challenge(self,i:u256):g=self._get(i);assert g.status==u8(CHALLENGED) and g.challenge_attempts<u8(MAX_RETRIES);self._apply(i,True)
 @gl.public.write
