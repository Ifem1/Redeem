...
class Redeem(gl.Contract):
    ...
    def create_guarantee(self,beneficiary:Address,title:str,terms:str,coverage_start:u256,coverage_end:u256,evaluation_earliest_at:u256,claim_deadline:u256,source_urls:str,outcome_codes:str,payout_bps:str):
        ...
        gid=self.next_id; self.next_id+=1; self.guarantees[gid]=Guarantee(gl.message.sender_address,beneficiary,title,terms,source_urls,outcome_codes,payout_bps,gl.message.value,u8(ACTIVE),False,coverage_start,coverage_end,evaluation_earliest_at,claim_deadline,"",u256(0),u256(0),Address.zero())
    ...
    def evaluate_redemption(self,guarantee_id:u256):
        ...
        parsed=json.loads(result); code=parsed["outcome_code"]; g.provisional_code=code; g.provisional_amount=self._payout(g,code); g.status=u8(PROVISIONAL); self.guarantees[guarantee_id]=g
    @gl.public.write.payable
    def challenge_redemption(self,guarantee_id:u256,reason:str):
        g=self._get(guarantee_id); assert g.status==u8(PROVISIONAL) and (gl.message.sender_address==g.issuer or gl.message.sender_address==g.beneficiary) and gl.message.value==g.escrow*u256(500)//u256(10000) and len(reason)<=500; g.status=u8(CHALLENGED); g.challenge_bond=gl.message.value; g.challenger=gl.message.sender_address; self.guarantees[guarantee_id]=g
    @gl.public.write
    def resolve_challenge(self,guarantee_id:u256):
        g=self._get(guarantee_id); assert g.status==u8(CHALLENGED)
        def second()->str:
            page=gl.nondet.web.render(g.sources.split("\n")[0],mode="text")[:4000]
        ...
        code=json.loads(result)["outcome_code"]; amount=self._payout(g,code); bond=g.challenge_bond; g.challenge_bond=u256(0); self._set(guarantee_id,g.beneficiary,amount,u8(PAID) if amount>0 else u8(DENIED)); self._pay(g.challenger if amount!=g.provisional_amount else (g.issuer if g.challenger==g.beneficiary else g.beneficiary),bond)
    @gl.public.write
    def finalize_redemption(self,guarantee_id:u256):
        g=self._get(guarantee_id); assert g.status==u8(PROVISIONAL); self._set(guarantee_id,g.beneficiary,g.provisional_amount,u8(PAID) if g.provisional_amount>0 else u8(DENIED))
    @gl.public.write
